...
import { brandLogoPath } from "@/lib/products/brand";
import { useCart } from "@/components/app-shell";

/** helper: ทำความสะอาด path รูปสินค้า */
function normalizeProductImageSrc(src?: string | null): string {
  if (!src || !src.trim()) {
  ...
  // ตัด /public ออก ถ้าเผลอเก็บมาจาก Laravel
  s = s.replace(/^\/public/, "");

  // ถ้ามี .jpg/.png/.webp หลายครั้ง → เอาถึงตัวแรกพอ
  const m = s.match(/^(.*?\.(?:jpe?g|png|gif|webp))/i);
  if (m) {
    s = m[1];
  }

  // ถ้ายังไม่มีนามสกุลรูปเลย → เติม .webp ให้
  if (!/\.(?:jpe?g|png|gif|webp)$/i.test(s)) {
    s = `${s}.webp`;
  }

  // ให้แน่ใจว่าขึ้นต้นด้วย /
  if (!s.startsWith("/")) {
    s = `/${s}`;
  }
...
export const ProductCard = ({
  ...
  // ✅ ใช้ state เก็บ src + fallback เป็น placeholder เวลา error
  ...
  const renderFrame = (roundClass: string) => {
    ...
    const draw = frameInfo as Extract<FrameInfo, { mode: "draw" }>;
    return (
      <div
        className={cn("pointer-events-none absolute inset-0", roundClass)}
        style={{
          zIndex: 5,
          border: `${draw.borderWidth}px solid ${draw.borderColorHex}`,
        }}
      />
    );
  };
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 h-full">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                {isInterlinkMall && (
                  <Badge variant="secondary">InterlinkMall</Badge>
                )}
                {isClearanceSale && (
                  <Badge variant="destructive">Clearance Sale</Badge>
                )}
              </div>
...
//   // ทำความสะอาด path รูป ที่ได้มาจาก API/DB
...
//           zIndex: 5,
//           border: `${
//             (frameInfo as Extract<FrameInfo, { mode: "draw" }>).borderWidth
//           }px solid ${
//             (frameInfo as Extract<FrameInfo, { mode: "draw" }>).borderColorHex
//           }`,
//         }}
//       />
//     );
