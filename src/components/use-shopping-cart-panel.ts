...
export type UICartItem = {
  ...
  /** 🔹 ใช้สำหรับ modal ให้แสดงสต๊อกตามเงื่อนไข CUT / ROLL */
  clearanceQuantity?: number | null;

  /** 🔹 เงื่อนไขการขาย (ใช้สร้างปุ่มความยาว ฯลฯ) */
  conditions?: any[] | null;

  /** 🔹 ข้อมูลเสริมสำหรับ CUT / ROLL (เผื่อใช้ใน modal) */
  salesMode?: "CUT" | "ROLL" | string | null;
...
type BackendCartItemWithProduct = BackendCartItem & {
  ...
  /** 👇 ถ้า backend ส่ง conditions มาก็รองรับไว้เลย */
  productConditions?: any[] | null;

  /** 👇 เผื่อ backend มี field พวกนี้ */
  productSalesMode?: string | null;
  productLengthPerItem?: number | null;
  productRollLength?: number | null;
};
    ...
    const loadCart = async () => {
      try {
        ...
        const mapped: UICartItem[] = apiItems.map((item) => {
          ...
          // 🔹 ดึง clearanceQuantity จาก backend (มีหลายชื่อสำรองไว้)
          const clearanceQuantityRaw =
            (item as any).productClearanceQuantity ??
            (item as any).clearanceQuantity ??
            null;

          const clearanceQuantity =
            clearanceQuantityRaw == null
              ? null
              : Number(clearanceQuantityRaw);

          // 🔹 ดึง conditions จาก backend (รองรับได้หลายชื่อ)
          const rawConditions =
          ...
          const conditions = Array.isArray(rawConditions)
            ? rawConditions
            : rawConditions
            ? [rawConditions] // ถ้าเป็น object เดียว ห่อเป็น array ให้
            : null;

          // 🔹 salesMode / lengthPerItem / rollLength (เผื่อ backend ส่งมา)
          const salesMode =
            (item as any).productSalesMode ??
            (item as any).salesMode ??
            null;

          const lengthPerItemRaw =
            (item as any).productLengthPerItem ??
            (item as any).lengthPerItem ??
            (item as any).cut_length ??
            null;

          const rollLengthRaw =
            (item as any).productRollLength ??
            (item as any).rollLength ??
            null;

          const lengthPerItem =
            lengthPerItemRaw == null ? null : Number(lengthPerItemRaw);
          const rollLength =
            rollLengthRaw == null ? null : Number(rollLengthRaw);
...
//           // 🔹 ดึง clearanceQuantity + conditions จาก backend (มีหลายชื่อสำรองไว้)
//           const clearanceQuantityRaw =
//             (item as any).productClearanceQuantity ??
//             (item as any).clearanceQuantity ??
//             null;
