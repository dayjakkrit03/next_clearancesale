...
// 🎯 นำเข้า Types + ฟังก์ชันจาก service layer (single source of truth)
import {
  getById,
  getMeta,
  type UIProduct,
...
// ✅ ใน Next 15 dynamic APIs อย่าง headers() กลายเป็น async → ต้อง await
...
async function fetchDiscountRules(): Promise<DiscountRuleLite[]> {
  ...
  const r = await fetch(`${base}/api/mock/discount-rules`, {
    cache: "no-store",
  });
...
export default async function Page({
  ...
  // 🎯 ใช้ UIProduct ที่ import จาก service layer
  const product: UIProduct | undefined = await getById(id);
