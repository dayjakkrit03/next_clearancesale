// v.1.1.4 ==============================================
// app/api/uploads/product_frame/route.ts
...
const UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "product_frame");

/** GET /api/uploads/product_frame?list=1 -> คืนลิสต์ไฟล์ PNG/WebP ที่มีอยู่ */
export async function GET(req: Request) {
  const { searchParams } = new URL(req.url);

  // ถ้าไม่ส่ง ?list=1 ก็แค่ตอบ ok (ไว้เช็คสุขภาพ route)
  if (searchParams.get("list") !== "1") {
    return NextResponse.json({ ok: true, hint: "use ?list=1 to list frames" });
  }
      ...
      await Promise.all(
        files
          ...
          .map(async (name) => {
            ...
            return {
              url: `/uploads/product_frame/${name}`,
              name,
              size: s?.size ?? null,
...
/** POST อัปโหลดไฟล์ PNG/WebP (เหมือนเดิม) */
