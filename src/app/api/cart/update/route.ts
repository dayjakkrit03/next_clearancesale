 ...
 * {
 *   id: number;            // cart id
 *   quantity: number;      // ค่าที่ต้องการเก็บใน field quantity (CUT/ROLL = totalLength)
 *   priceAmount: number;   // ราคารวม (เก็บลง price_amount)
 * }
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const priceAmount =
      body.priceAmount != null ? Number(body.priceAmount) : undefined;

    if (!Number.isFinite(quantity!) || !Number.isFinite(priceAmount!)) {
      return NextResponse.json(
        { status: "error", message: "invalid quantity or priceAmount" },
        { status: 400 },
      );
    }
    ...
    if (!customerId) {
      return NextResponse.json(
        { status: "login", message: "unauthorized" },
        { status: 200 },
      );
    }
    ...
    // อัปเดตเฉพาะแถวนี้ (สมมติ id เป็น primary key)
    await prismaShop.carts.update({
      where: {
        id: BigInt(id),
      },
      data: {
        // quantity เก็บเป็นจำนวนรวมตามที่ frontend คำนวณแล้ว
        quantity,
        // price_amount เก็บราคารวมของรายการนี้
