...
import { setInterlinkSessionTZ } from "@/lib/db";
...
// ✅ ใช้ helper กลางจาก src/lib/image-path.ts
import {
  normalizeBasePath as normalizeBasePathLib,
 ...
 * ใช้ implementation กลางจาก src/lib/image-path.ts
 */
export function normalizeBasePath(p?: string | null) {
  return normalizeBasePathLib(p);
}
...
export function mapRowToUI(r: any): UIProduct {
  // ✅ ใช้ helper กลางจาก src/lib/image-path.ts
  // priority: image_name จาก images_products -> product_filename -> null
  const finalImageUrl =
    buildProductImagePath(
      r.image_url,
      r.image_name ?? r.product_filename ?? null
    ) ?? undefined;
